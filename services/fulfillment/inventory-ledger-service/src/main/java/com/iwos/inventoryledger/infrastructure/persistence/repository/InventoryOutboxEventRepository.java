...
public interface InventoryOutboxEventRepository extends JpaRepository<InventoryOutboxEventEntity, UUID> {
    ...
    List<InventoryOutboxEventEntity> findByStatusOrderByCreatedAtAsc(String status);

    long countByStatus(String status);

    @Modifying
    @Transactional
    @Query("UPDATE InventoryOutboxEventEntity e SET e.status = :status WHERE e.outboxEventId = :id")
