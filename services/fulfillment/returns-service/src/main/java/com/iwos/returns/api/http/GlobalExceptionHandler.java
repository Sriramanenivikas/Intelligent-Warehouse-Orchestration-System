...
public class GlobalExceptionHandler {
    ...
    public ApiErrorResponse handleNotFound(ReturnRequestNotFoundException exception, HttpServletRequest request) {
        return error(HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND", exception.getMessage(), request.getRequestURI());
    }

    @ExceptionHandler(ReturnRequestStatusException.class)
    @ResponseStatus(HttpStatus.CONFLICT)
    public ApiErrorResponse handleConflict(ReturnRequestStatusException exception, HttpServletRequest request) {
        return error(HttpStatus.CONFLICT, "INVALID_RETURN_STATE", exception.getMessage(), request.getRequestURI());
    }
